...
```
Nnecessary paramters: As the name suggests `--unseen_classname` is to prove name of unseen class like `tunnel`, `limit` is the number of images to donaload for training, top is the number of seen classes UnseenNet will use to perform Adaptation (0<top<=100), `chromedriver` is to give the location of driver to donalod images. 
